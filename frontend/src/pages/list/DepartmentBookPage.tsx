import { getTeamBooks } from '@apis/api';
...
const DepartmentBookPage = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (location.search) {
          ...
          bookListData = await getTeamBooks({
            page: 1,
            size: 20,
            type: 2,
            teamId: 1,
  ...
  const fetchNextPageData = async () => {
    try {
      setPage((prev) => prev + 1);
      const bookListData = await getTeamBooks({
        type: 2,
        page: page,
        size: 20,
        teamId: 1,
...
export default DepartmentBookPage;
